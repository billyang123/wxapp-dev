...
Component.prototype.setData = function setData(data, value) {
  ...
  if (false) {
    ...
    console.log('%c%s setData(%o) : %o -> %o Component:%o %s', 'color:#' + (changed ? '2a8f99' : 'bbb'), me.id, append, original, JSON.parse((0, _stringify2.default)(this.data)), me, changed ? '' : 'Unchanged');
  } else {
    this.data = (0, _assign2.default)({}, this.data, data);
  }
...
Component.prototype._init = function (key, parent) {
  ...
  //初始化props
  if (me._props) {
    (0, _keys2.default)(me._props).forEach(function (k) {
      var v = me._props[k];
      //console.log('k', '->', k, v);
      if (typeof v === 'string') {
  ...
  if (false && me.propTypes) {
    (0, _keys2.default)(me.propTypes).forEach(function (propName) {
